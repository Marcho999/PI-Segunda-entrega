...
public class TiendaBoutiqueCompleta extends JFrame {
    ...
    private void initComponents() {
        ...
        panelDashboard.setLayout(new GridLayout(3, 2, 15, 15));
        ...
        modeloInventario = new DefaultTableModel(new String[]{"ID", "Producto", "Talla", "Precio", "Stock"}, 0);
        JTable tablaInventarioDashboard = crearTabla(modeloInventario);
        ...
        panelInventario.add(new JScrollPane(tablaInventarioDashboard), BorderLayout.CENTER);
        ...
        // ======== INVENTARIO ADMIN ========
        JPanel panelInventarioAdmin = crearPanel(Color.decode("#F0F8FF"));
        modeloInventario = new DefaultTableModel(new String[]{"ID", "Nombre", "Talla", "Precio", "Stock"}, 0);
        JTable tablaInventarioAdmin = crearTabla(modeloInventario);
        panelInventarioAdmin.add(new JScrollPane(tablaInventarioAdmin), BorderLayout.CENTER);

        JPanel panelBotonesInventario = new JPanel();
        JButton btnAgregarInventario = crearBoton("Agregar", "#4682B4");
        JButton btnActualizarInventario = crearBoton("Actualizar", "#5F9EA0");
        JButton btnEliminarInventario = crearBoton("Eliminar", "#CD5C5C");
        JButton btnRefrescarInventario = crearBoton("Refrescar", "#FF8C00");
        panelBotonesInventario.add(btnAgregarInventario);
        panelBotonesInventario.add(btnActualizarInventario);
        panelBotonesInventario.add(btnEliminarInventario);
        panelBotonesInventario.add(btnRefrescarInventario);
        panelInventarioAdmin.add(panelBotonesInventario, BorderLayout.SOUTH);

        // Acciones de botones
        btnRefrescarInventario.addActionListener(e -> listarInventarioAdmin());
        btnAgregarInventario.addActionListener(e -> agregarProductoInventario());
        btnActualizarInventario.addActionListener(e -> actualizarProductoInventario(tablaInventarioAdmin));
        btnEliminarInventario.addActionListener(e -> eliminarProductoInventario(tablaInventarioAdmin));

        tabbedPane.addTab("Inventario", panelInventarioAdmin);
    ...
    private void listarInventarioAdmin() {
        modeloInventario.setRowCount(0);
        String sql = "SELECT i.inventario_id, p.nombre, i.talla, i.precio_unitario, i.stock FROM Inventario i JOIN Producto p ON i.producto_id = p.idproducto";
        try (Statement st = conexion.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
    ...
    private void agregarProductoInventario() {
        String idProducto = JOptionPane.showInputDialog("ID Producto:");
        String talla = JOptionPane.showInputDialog("Talla:");
        String precioStr = JOptionPane.showInputDialog("Precio:");
        String stockStr = JOptionPane.showInputDialog("Stock:");

        if (idProducto != null && talla != null && precioStr != null && stockStr != null) {
            try {
                double precio = Double.parseDouble(precioStr);
                int stock = Integer.parseInt(stockStr);
                String sql = "INSERT INTO Inventario(producto_id, talla, precio_unitario, stock) VALUES (?, ?, ?, ?)";
                try (PreparedStatement ps = conexion.prepareStatement(sql)) {
                ...
                mostrarError("agregar inventario", e);
            }
        }
    }

    private void actualizarProductoInventario(JTable tabla) {
        int fila = tabla.getSelectedRow();
        if (fila >= 0) {
            String idInventario = modeloInventario.getValueAt(fila, 0).toString();
            String talla = JOptionPane.showInputDialog("Talla:", modeloInventario.getValueAt(fila, 2));
            String precioStr = JOptionPane.showInputDialog("Precio:", modeloInventario.getValueAt(fila, 3));
            String stockStr = JOptionPane.showInputDialog("Stock:", modeloInventario.getValueAt(fila, 4));
            try {
                double precio = Double.parseDouble(precioStr);
                int stock = Integer.parseInt(stockStr);
                String sql = "UPDATE Inventario SET talla = ?, precio_unitario = ?, stock = ? WHERE inventario_id = ?";
                try (PreparedStatement ps = conexion.prepareStatement(sql)) {
                    ps.setString(1, talla);
                    ps.setDouble(2, precio);
                    ps.setInt(3, stock);
                    ps.setInt(4, Integer.parseInt(idInventario));
                    ps.executeUpdate();
                    listarInventarioAdmin();
                    JOptionPane.showMessageDialog(this, "Producto actualizado.");
                }
            } catch (NumberFormatException | SQLException e) {
                mostrarError("actualizar inventario", e);
            }
        } else {
    ...
    private void eliminarProductoInventario(JTable tabla) {
        int fila = tabla.getSelectedRow();
        if (fila >= 0) {
            int id = (int) modeloInventario.getValueAt(fila, 0);
            int confirm = JOptionPane.showConfirmDialog(this, "¿Eliminar producto del inventario?");
            if (confirm == JOptionPane.YES_OPTION) {
                try (PreparedStatement ps = conexion.prepareStatement("DELETE FROM Inventario WHERE inventario_id = ?")) {
                    ps.setInt(1, id);
                    ps.executeUpdate();
                    listarInventarioAdmin();
                    JOptionPane.showMessageDialog(this, "Producto eliminado.");
                } catch (SQLException e) {
                    mostrarError("eliminar inventario", e);
                }
            }
        } else {
            JOptionPane.showMessageDialog(this, "Seleccione un producto.");
